...
sub processdata($local) {
    ...
    say "local is $servavail",$local.WHAT;
    react {
        ...
        whenever $local.Supply(:bin) -> $reply {
            say "have reply";
            say $reply.WHAT,$reply.decode;
            #say $reply.decode;
            #say Blob.new($reply[1..$reply[0]]).decode;
            $recvdata.emit($reply);
        }
    }
}
            ...
            react {
                whenever $local.Supply -> $data {   
                    given $data {
                        ...
                        default {
                            say |@localinfo,' says:',$check ~ $data;
                            $local.print("Sorry\0");
                            $local.close;
                            say 'Sorry, ',|@localinfo, ' is not magic.';
                            done ;
                        }
                    }
                }
                whenever $checkcli -> $check {
                    given $check {
                        when $hello ~ $magic { 
                            say |@localinfo, ' is magic';
                            $local.print("Oh! Magic.\0");
                            $servavail = True;
                        ...
                        when 'time out' {
                            say |@localinfo, ' time out';
                            $local.print("Time out\0");
                            $local.close;
                            done ;
                        }
                        default {
                            say |@localinfo, ' Missspeak';
                            $local.print("Missspeak\0");
                            $local.close;
                            done ;
                        }
                    }
                }
                whenever Supply.interval(1,20) {
                   $start.emit('time out');
                }
            }      
        }
